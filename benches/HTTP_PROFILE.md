...
| **Path** | Fix (micro-opts #2, #4, then serialize #3 / PR4) |
| **Not done** | Body double-materialization (#1 / PR3 open), conn assembly reuse (PR5), public API, reactor, HTTP/2 |
| **Follow-up** | Epic **v0.8** / [#16](https://github.com/Tuntii/viltrum/issues/16) (PR3, PR5–PR7). Baseline vs Axum: `benches/compare/` |
...
3. **Response `[]u8` builder + header casing cache (PR4 / #20)** — `to_bytes_for_method` writes a pre-sized byte buffer (no growing string + `out.bytes()`). Known headers use fixed wire casing; `canonicalize_header_name` only for custom fields. HEAD check is case-insensitive without full `to_upper`.

**Tests:** `v test http/ router/ engine/ ws/` — green (incl. wire-casing / HEAD case tests).
