...
// Minimal cleartext HTTP/1.1 client. One request per TCP connection (Connection: close).
// Not a general client library: no TLS, redirects, cookies, or pooling.
...
fn host_from_addr(addr string) string {
	// strip IPv6 brackets if present; keep host:port as Host value is fine for tests
	return addr
}
...
fn read_client_response(mut conn net.TcpConn) !Response {
	...
	for {
		...
		if sep := index_of_double_crlf(buf) {
			...
			need := hdr_end + cl
			for buf.len < need {
				n2 := conn.read(mut tmp) or { return err }
				if n2 <= 0 {
